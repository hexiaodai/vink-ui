...
export default () => {
    ...
    const [selected, setSelected] = useState<{ dataDisks: DataVolume[] | undefined, networks: VirtualMachineNetwork[] | undefined }>({ dataDisks: undefined, networks: undefined })
    ...
    const networkColumns: TableProps<VirtualMachineNetwork>['columns'] = [
        {
            title: '网络',
            key: 'network',
            ellipsis: true,
        ...
        },
        {
            ...
            key: 'ipAddress',
            ellipsis: true,
            render: (_, cfg) => cfg.ip || "自动分配"
        },
        {
            title: 'MAC 地址',
            key: 'macAddress',
            ellipsis: true,
            render: (_, cfg) => cfg.mac || "自动分配"
        },
        {
            title: '子网',
            key: 'subnet',
            ellipsis: true,
            render: (_, cfg) => cfg.subnet
        },
        {
            title: 'VPC',
            key: 'vpc',
            ellipsis: true,
            render: (_, cfg) => cfg.vpc
        },
        {
            title: 'Multus',
            key: 'multus',
            ellipsis: true,
            render: (_, cfg) => cfg.multus
        },
        {
            title: '操作',
            key: 'action',
    ...
    return (
        <>
            ...
            <NetworkDrawer
                ...
                onConfirm={(data) => {
                    const networks = formRef.current?.getFieldsValue().networks || []
                    const hasElement = networks.some((cfg: VirtualMachineNetwork) => {
                        return cfg.multus == data.multus
                    })
                    if (hasElement) {
                        return
                    }
